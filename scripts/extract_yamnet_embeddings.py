...
def load_wav_16k(path):
    ...
    if sr_int != 16000:
        ...
        target_length = max(1, round(len(audio_np) * 16000 / sr_int))
        source_positions = np.linspace(0.0, 1.0, num=len(audio_np), endpoint=False)
        target_positions = np.linspace(0.0, 1.0, num=target_length, endpoint=False)
        audio = tf.convert_to_tensor(
            np.interp(target_positions, source_positions, audio_np),
            dtype=tf.float32,
        )
